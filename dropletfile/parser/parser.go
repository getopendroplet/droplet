// Package parser implements a parser and parse tree dumper for Dropletfiles.
package parser
...
// but lucky for us the Dropletfile isn't very complicated. This structure
...
type Node struct {
	...
	StartLine   int             // the line in the original dropletfile where the node begins
	EndLine     int             // the line in the original dropletfile where the node ends
	PrevComment []string
}
...
// continuation token in a Dropletfile. Only ` (backtick) and \ (backslash) are
// allowed as token.
...
func init() {
	...
	dispatch = map[string]func(string, *directives) (*Node, map[string]bool, error){
		...
		command.Config:  parseStringsWhitespaceDelimited,
		command.Copy:    parseMaybeJSONToList,
		command.Cron:    parseMaybeJSONToList,
		command.Delete:  parseMaybeJSONToList,
		command.Env:     parseEnv,
		command.Expose:  parseStringsWhitespaceDelimited,
		command.Label:   parseLabel,
		command.Package: parseMaybeJSONToList,
		command.Run:     parseMaybeJSON,
		command.Stage:   parseStringsWhitespaceDelimited,
...
func newNodeFromLine(line string, d *directives, comments []string) (*Node, error) {
	...
	fn := dispatch[cmd]
	// Ignore invalid Dropletfile instructions
	if fn == nil {
		fn = parseIgnore
	}
...
// Result is the result of parsing a Dropletfile
type Result struct {
	AST         *Node
	EscapeToken rune
	Warnings    []string
...
func handleScannerError(err error) error {
	...
	case bufio.ErrTooLong:
		return errors.Errorf("dropletfile line greater than max allowed size of %d", bufio.MaxScanTokenSize-1)
	default:
		return err
	}
}
