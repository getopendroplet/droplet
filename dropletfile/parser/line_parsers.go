...
// Node object which contains the whole statement. Dropletfiles have varied
...
var (
	errDropletfileNotStringArray = errors.New("when using JSON array syntax, arrays must be comprised of strings only")
)
...
func parseJSON(rest string, d *directives) (*Node, map[string]bool, error) {
	...
	for _, str := range myJSON {
		s, ok := str.(string)
		if !ok {
			return nil, nil, errDropletfileNotStringArray
		}
...
func parseMaybeJSON(rest string, d *directives) (*Node, map[string]bool, error) {
	...
	if err == errDropletfileNotStringArray {
		return nil, nil, err
	}
